--[[
    OBFUSCATE FIRST, or everyone will see your source
]]
local function PSU_CRASH() print("crash " .. debug.traceback()) end;
local function PSU_ENCRYPT_STRING(f) return f end;
local function PSU_FAST_EXECUTION(f) return f() end;
local function PSU_MAX_SECURITY_START() end;
local function PSU_MAX_SECURITY_END() end;

-- stops the script getting executed more than once
if (shared.hydra) then
    return;
else
    shared.hydra = true;
end;
...
local whitelist, completedChecks, httpPost, httpGet; do
    whitelist = {
        hmacKey = PSU_ENCRYPT_STRING("0DKsaIxBg87!£y9nXY1Y5#$bsJV&6SyjBUKirhZp2wo9Cz3!SNCiMd1R$22HwR");
    };

    whitelist.hash = (function()
        local Base64 = {
...
-- print(whitelist.hash.hmac(whitelist.hash.sha3_256, whitelist.hmacKey, "testing"));
-- repeat wait() until completedChecks;

local succ, err = pcall(function()
    -- server is down or something
    local succ, res = pcall(httpGet, PSU_ENCRYPT_STRING("http://localhost:3000/"));
    if (not succ) then
        client:Kick("Failed to connect to webserver.");
    else
        ...
        if (not successful) then
            ...
            local antijmp; do
                local tostringhook;
                tostringhook = hookfunc(tostring, newcclosure(function(self)
                    if (tostringhook(self):find("localhost")) then -- obviously change all of these to the actual URL
                        PSU_CRASH();
                        return "";
                    end;

                    return tostringhook(self);
                end));

                if (is_synapse_function(httpService.JSONEncode) or is_synapse_function(httpService.JSONDecode)) then
                    PSU_CRASH();
                end;

                if (is_synapse_function(string.format) or 1 + 1 ~= 2) then
                    PSU_CRASH();
                end;
                
                local yes, no = pcall(debug.getconstants, http_request);
                if (not no == "bad argument #1 to \'?\' (Lua function expected)" or yes) then
                    PSU_CRASH();
                end;

                if (islclosure(string.format) or is_synapse_function(string.format) or pcall(getconstants, string.format)) then
                    PSU_CRASH();
                end;
                antijmp = true;
            end;

            repeat wait() until antijmp;
            local hmac, sha3, base64_encode, base64_decode = whitelist.hash.hmac, whitelist.hash.sha3_256, whitelist.hash.base64_encode, whitelist.hash.base64_decode;

            local request = httpPost(PSU_ENCRYPT_STRING("http://localhost:3000/auth"), {
                [hmac(sha3, signature .. "DM Spencer#0003 for free VBucks" .. whitelist.hmacKey, PSU_ENCRYPT_STRING("Key"))] = base64_encode(hmac(sha3, signature, key));
                [hmac(sha3, whitelist.hmacKey .. signature .. "https://tenor.com/view/doing-ur-mom-gif-18200972", PSU_ENCRYPT_STRING("ScriptID"))] = base64_encode(scriptID);
                [hmac(sha3, signature .. "Doing your mom" .. whitelist.hmacKey, PSU_ENCRYPT_STRING("Auth"))] = base64_encode(hmac(sha3, whitelist.hmacKey .. signature, "ICEE plays Fortnite."));
                [hmac(sha3, whitelist.hmacKey, PSU_ENCRYPT_STRING("Signature"))] = base64_encode(signature);
                [hmac(sha3, whitelist.hmacKey, "Roblox Name")] = base64_encode(client.Name);
            });

            local response = httpService:JSONDecode(request.Body);
            local decodedResponse = base64_decode(response.body);
            local clientExcpects = hmac(sha3, whitelist.hmacKey .. signature, (scriptID .. "Valid"));

            if (response.success) then
                if (decodedResponse == clientExcpects and rawequal(decodedResponse, clientExcpects) and (decodedResponse:len() == 64) and (clientExcpects:len() == 64) and decodedResponse:find(clientExcpects) and (typeof(clientExcpects) == "string") and (typeof(decodedResponse) == "string")) then
                    -- whitelist has passed, put your script here basically
                    print("Success");
                else
                    print("Failed");
                end;
